...
function currencyChecker(tomaat) {
    ...
    if (tomaat.length === 2) return output + `${tomaat[0].name} and ${tomaat[1].name}`;
    else return output + `${tomaat[0].name};`;
}
...
function languageChecker(banaan) {
    if (banaan.length >= 2) return `${banaan[0].name} and ${banaan[1].name} and ${banaan[2].name}`;
    else return `${banaan[0].name}`;
}
